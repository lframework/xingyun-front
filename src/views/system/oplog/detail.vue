  ...
  <a-modal v-model="visible" :mask-closable="false" width="50%" title="查看" :dialog-style="{ top: '20px' }" :footer="null">
    <div v-if="visible" v-permission="['system:oplog:query']" v-loading="loading">
      <a-descriptions bordered :xs="4" :sm="2" :md="2">
        ...
        <a-descriptions-item label="日志类型" :span="2">{{ $enums.OP_LOG_TYPE.getDesc(formData.logType) }}</a-descriptions-item>
        ...
        <a-descriptions-item label="补充信息" :span="4"><a-textarea v-if="!$utils.isEmpty(formData.extra)" v-model="formData.extra" read-only @click="handleClipboard(formData.extra, $event)" /></a-descriptions-item>
      </a-descriptions>
    </div>
  </a-modal>
</template>
