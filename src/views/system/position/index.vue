<template>
  <div v-permission="['system:position:query']">
      ...
      >
        ...
        <template #toolbar_buttons>
          <a-space>
            ...
            <a-button
              v-permission="['system:position:add']"
              type="primary"
              :icon="h(PlusOutlined)"
    ...
    methods: {
      ...
      createActions(row) {
        return [
          ...
          {
            permission: ['system:position:modify'],
            label: '修改',
            onClick: () => {
              this.id = row.id;
      ...
      batchUnable() {
        ...
        if (this.$utils.isEmpty(records)) {
          this.$msg.createError('请选择要停用的岗位！');
          return;
        }

        this.$msg.createConfirm('是否确定停用选择的岗位？').then(() => {
          this.loading = true;
      ...
      batchEnable() {
        ...
        if (this.$utils.isEmpty(records)) {
          this.$msg.createError('请选择要启用的岗位！');
          return;
        }

        this.$msg.createConfirm('是否确定启用选择的岗位？').then(() => {
          this.loading = true;
          const ids = records.map((t) => t.id);
          api
            .batchEnable(ids)
