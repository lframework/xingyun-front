                  ...
                  <a-dropdown>
                    <template #overlay>
                      <a-menu @click="handleCommand">
                        ...
                        <a-menu-item key="batchUnable" :icon="h(StopOutlined)">
                          批量停用
                        </a-menu-item>
                      </a-menu>
                    </template>
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      batchUnable() {
        ...
        this.$msg.createConfirm('是否确定停用选择的数据对象？').then(() => {
          this.loading = true;
      ...
      batchEnable() {
        ...
        this.$msg.createConfirm('是否确定启用选择的数据对象？').then(() => {
          this.loading = true;
          const ids = records.map((t) => t.id);
          api
            .batchEnable(ids)
      ...
      batchDelete() {
        ...
        this.$msg.createConfirm('是否确定删除选择的数据对象？').then(() => {
          this.loading = true;
          const ids = records.map((t) => t.id);
          api
            .batchDelete(ids)
