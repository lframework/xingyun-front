  ...
  export default defineComponent({
    ...
    data() {
      return {
        ...
        tableColumn: [
          ...
          { title: '操作', width: 310, fixed: 'right', slots: { default: 'action_default' } },
        ],
        // 请求接口配置
        proxyConfig: {
          props: {
    ...
    methods: {
      ...
      batchUnable() {
        ...
        this.$msg.createConfirm('是否确定停用选择的数据实体？').then(() => {
          this.loading = true;
      ...
      batchEnable() {
        ...
        this.$msg.createConfirm('是否确定启用选择的数据实体？').then(() => {
          this.loading = true;
          const ids = records.map((t) => t.id);
          api
            .batchEnable(ids)
      ...
      batchDelete() {
        ...
        this.$msg.createConfirm('是否确定删除选择的数据实体？').then(() => {
          this.loading = true;
          const ids = records.map((t) => t.id);
          api
            .batchDelete(ids)
        ...
        return [
          ...
          },
          {
            ...
            onClick: () => {
              this.id = row.id;
              this.visible = false;
              this.$nextTick(() => this.$refs.generateDialog.openDialog());
            },
          },
          {
            label: '预览',
            ...
            onClick: () => {
              this.id = row.id;
              this.visible = false;
              this.$nextTick(() => this.$refs.previewDialog.openDialog());
            },
          },
          {
            label: '下载',
