  ...
  <div>
    ...
    >
      ...
      <template #width_default="{ row }">
        <a-input-number v-model:value="row.width" class="number-input" :min="0" :precision="0" />
      </template>
      ...
      <template #btnConfig_default="{ row }">
        ...
        />
        <a-input
          v-else-if="$enums.GEN_CUSTOM_LIST_BTN_TYPE.ROUTE.equalsCode(row.btnType)"
          v-model:value="row.btnConfig"
        />
        <a
          v-else-if="$enums.GEN_CUSTOM_LIST_BTN_TYPE.EXCUTE_SCRIPT.equalsCode(row.btnType)"
