<template>
  <div>
    <div v-permission="['customer-settle:fee-sheet:query']">
        ...
        >
              ...
              <j-form bordered @collapse="$refs.grid.refreshColumn()">
                ...
                <j-form-item label="状态">
                  <a-select v-model:value="searchFormData.status" placeholder="全部" allow-clear>
                    <a-select-option
                      v-for="item in $enums.CUSTOMER_SETTLE_FEE_SHEET_STATUS.values()"
                      :key="item.code"
                      :value="item.code"
          ...
          <template #toolbar_buttons>
            <a-space>
              ...
              >
              <a-button
                v-permission="['customer-settle:fee-sheet:approve']"
              ...
              >
              <a-button
                v-permission="['customer-settle:fee-sheet:delete']"
                danger
              ...
              >
              <a-button
                v-permission="['customer-settle:fee-sheet:export']"
...
<script>
  ...
  export default defineComponent({
      ...
      return {
        ...
        tableColumn: [
          ...
          { field: 'createBy', title: '操作人', width: 100 },
          {
            ...
            width: 100,
            formatter: ({ cellValue }) => {
              return this.$enums.CUSTOMER_SETTLE_FEE_SHEET_STATUS.getDesc(cellValue);
            },
          },
    ...
    methods: {
      ...
      // 删除订单
      deleteOrder(row) {
        createConfirm('对选中的费用单执行删除操作？').then(() => {
          this.loading = true;
          api
            .deleteById(row.id)
            .then(() => {
      ...
      batchDelete() {
        ...
        if (isEmpty(records)) {
          createError('请选择要执行操作的费用单！');
          return;
        }

        for (let i = 0; i < records.length; i++) {
          if (
            this.$enums.CUSTOMER_SETTLE_FEE_SHEET_STATUS.APPROVE_PASS.equalsCode(records[i].status)
          ) {
            createError('第' + (i + 1) + '个费用单已审核通过，不允许执行删除操作！');
            return;
          }
        }
      ...
      batchApprovePass() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (
            ...
            createError('第' + (i + 1) + '个费用单已审核通过，不允许继续执行审核！');
            return;
          }
        }
      ...
      batchApproveRefuse() {
        ...
        for (let i = 0; i < records.length; i++) {
          ...
          if (
            this.$enums.CUSTOMER_SETTLE_FEE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(
              records[i].status,
            )
          ) {
            createError('第' + (i + 1) + '个费用单已审核拒绝，不允许继续执行审核！');
            return;
          }
        }
      ...
      createActions(row) {
        return [
          ...
          {
            permission: ['customer-settle:fee-sheet:approve'],
            label: '审核',
            ifShow: () => {
              return (
                this.$enums.CUSTOMER_SETTLE_FEE_SHEET_STATUS.CREATED.equalsCode(row.status) ||
                this.$enums.CUSTOMER_SETTLE_FEE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)
              );
            },
            onClick: () => {
          ...
          {
            permission: ['customer-settle:fee-sheet:modify'],
            label: '修改',
            ifShow: () => {
              return (
                this.$enums.CUSTOMER_SETTLE_FEE_SHEET_STATUS.CREATED.equalsCode(row.status) ||
          ...
          {
            permission: ['customer-settle:fee-sheet:delete'],
            label: '删除',
            danger: true,
            ifShow: () => {
              return (
