<template>
  <div>
    <div v-permission="['customer-settle:check-sheet:query']" class="app-container">
      <!-- 数据列表 -->
      <vxe-grid
        id="CustomerSettleCheckSheet"
        ref="grid"
        resizable
        show-overflow
        highlight-hover-row
      ...
      >
        ...
        <template v-slot:toolbar_buttons>
          <a-space>
            ...
            <a-button v-permission="['customer-settle:check-sheet:add']" type="primary" icon="plus" @click="$router.push('/settle/customer/check-sheet/add')">新增</a-button>
            <a-button v-permission="['customer-settle:check-sheet:approve']" icon="check" @click="batchApprovePass">审核通过</a-button>
            <a-button v-permission="['customer-settle:check-sheet:approve']" icon="close" @click="batchApproveRefuse">审核拒绝</a-button>
            <a-button v-permission="['customer-settle:check-sheet:delete']" type="danger" icon="delete" @click="batchDelete">批量删除</a-button>
            <a-button v-permission="['customer-settle:check-sheet:export']" icon="download" @click="exportList">导出</a-button>
          </a-space>
        </template>

        <!-- 操作 列自定义内容 -->
        <template v-slot:action_default="{ row }">
          <a-button v-permission="['customer-settle:check-sheet:query']" type="link" @click="e => { id = row.id;$nextTick(() => $refs.viewDialog.openDialog()) }">查看</a-button>
          <a-button v-if="$enums.CUSTOMER_SETTLE_CHECK_SHEET_STATUS.CREATED.equalsCode(row.status) || $enums.CUSTOMER_SETTLE_CHECK_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['customer-settle:check-sheet:approve']" type="link" @click="$router.push('/settle/customer/check-sheet/approve/' + row.id)">审核</a-button>
          <a-button v-if="$enums.CUSTOMER_SETTLE_CHECK_SHEET_STATUS.CREATED.equalsCode(row.status) || $enums.CUSTOMER_SETTLE_CHECK_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['customer-settle:check-sheet:modify']" type="link" @click="$router.push('/settle/customer/check-sheet/modify/' + row.id)">修改</a-button>
          <a-button v-if="$enums.CUSTOMER_SETTLE_CHECK_SHEET_STATUS.CREATED.equalsCode(row.status) || $enums.CUSTOMER_SETTLE_CHECK_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['customer-settle:check-sheet:delete']" type="link" class="ant-btn-link-danger" @click="deleteOrder(row)">删除</a-button>
        </template>
      </vxe-grid>
