<template>
  <a-modal
    ...
    :footer="null"
  >
    <div v-if="visible" v-permission="['base-data:stock-cell:add']" v-loading="loading">
      <a-form
        ref="form"
        :label-col="{ span: 4 }"
        :wrapper-col="{ span: 16 }"
      ...
      >
        ...
        <a-form-item label="仓位类别" name="cellType">
          <a-select v-model:value="formData.cellType" allow-clear>
            <a-select-option
    ...
    data() {
      return {
        ...
        rules: {
          ...
          cellType: [{ required: true, message: '请选择仓位类别' }],
        },
      };
    },
    computed: {},
    ...
    methods: {
      ...
      submit() {
        this.$refs.form.validate().then((valid) => {
          if (valid) {
            this.loading = true;
            const params = Object.assign({}, this.formData, { scId: this.scId });
            api
              .create(params)
              .then(() => {
                createSuccess('新增成功！');
      ...
      onGenerateCode() {
        generateCode(GENERATE_CODE_TYPE.STOCK_CELL.code).then((res) => {
