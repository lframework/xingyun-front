<template>
  <div v-permission="['base-data:member:query']" class="app-container">
    ...
    >
      ...
      <template v-slot:toolbar_buttons>
        <el-form :inline="true">
          ...
          <el-form-item v-permission="['base-data:member:modify']">
      ...
      <template v-slot:action_default="{ row }">
        <el-button v-permission="['base-data:member:query']" type="text" icon="el-icon-view" @click="e => { id = row.id;$refs.viewDialog.openDialog() }">查看</el-button>
        <el-button v-permission="['base-data:member:modify']" type="text" icon="el-icon-edit" @click="e => { id = row.id;$refs.updateDialog.openDialog() }">修改</el-button>
      </template>
    </vxe-grid>
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({ page, sorts, filters }) => {
            return this.$api.baseData.member.query(this.buildQueryParams(page))
          }
        }
      }
    }
  ...
  methods: {
    ...
    batchUnable() {
      ...
      if (this.$utils.isEmpty(records)) {
        this.$msg.error('请选择要停用的会员！')
        return
      }

      this.$msg.confirm('是否确定停用选择的会员？').then(() => {
        this.loading = true
        const ids = records.map(t => t.id)
        this.$api.baseData.member.batchUnable(ids).then(data => {
    ...
    batchEnable() {
      ...
      if (this.$utils.isEmpty(records)) {
        this.$msg.error('请选择要启用的会员！')
        return
      }

      this.$msg.confirm('是否确定启用选择的会员？').then(() => {
        this.loading = true
        const ids = records.map(t => t.id)
        this.$api.baseData.member.batchEnable(ids).then(data => {
