...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        code: [
          { required: true, message: '请输入销售属性组编号' }
        ],
        name: [
          { required: true, message: '请输入销售属性组名称' }
        ],
        available: [
  ...
  methods: {
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.loading = true
          this.$api.baseData.product.salePropGroup.modify(this.formData).then(() => {
    ...
    async loadFormData() {
      this.loading = true
      await this.$api.baseData.product.salePropGroup.get(this.id).then(data => {
        this.formData = data
      }).finally(() => {
        this.loading = false
      })
