  ...
  <div v-permission="['base-data:product:property:query']" class="app-container">
    ...
    >
      <template v-slot:form>
        <j-border>
          <j-form>
            <j-form-item label="属性编号" :span="6">
              <el-input v-model="searchFormData.code" clearable />
            </j-form-item>
            <j-form-item label="属性名称" :span="6">
      ...
      <template v-slot:toolbar_buttons>
        <el-form :inline="true">
          ...
          <el-form-item v-permission="['base-data:product:property:add']">
            <el-button type="primary" icon="el-icon-plus" @click="$refs.addDialog.openDialog()">新增</el-button>
          </el-form-item>
          <el-form-item v-permission="['base-data:product:property:modify']">
      ...
      <template v-slot:action_default="{ row }">
        <el-button v-if="!$enums.COLUMN_TYPE.CUSTOM.equalsCode(row.columnType)" v-permission="['base-data:product:property-item:query']" type="text" icon="el-icon-setting" @click="e => { id = row.id;$refs.itemDialog.openDialog() }">属性值管理</el-button>
        <el-button v-permission="['base-data:product:property:modify']" type="text" icon="el-icon-edit" @click="e => { id = row.id;$refs.updateDialog.openDialog() }">修改</el-button>
      </template>
    </vxe-grid>
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        { field: 'code', title: '属性编号', width: 120 },
        { field: 'name', title: '属性名称', minWidth: 160 },
        ...
        { field: 'propertyType', title: '属性类别', width: 100, formatter: ({ cellValue }) => { return this.$enums.PROPERTY_TYPE.getDesc(cellValue) } },
