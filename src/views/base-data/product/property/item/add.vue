<template>
  <a-modal v-model="visible" :mask-closable="false" width="40%" title="新增" :dialog-style="{ top: '20px' }" :footer="null">
    <div v-if="visible" v-permission="['base-data:product:property-item:add']" v-loading="loading">
  ...
  methods: {
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.loading = true
          this.$api.baseData.product.propertyItem.create(Object.assign({ propertyId: this.propertyId }, this.formData)).then(() => {
