<template>
  <el-dialog :visible.sync="visible" :close-on-click-modal="false" :append-to-body="true" width="40%" top="5vh" title="修改" @open="open">
    <div v-if="visible" v-permission="['base-data:product:property-item:modify']">
  ...
  methods: {
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.loading = true
          this.$api.baseData.product.propertyItem.modify(Object.assign({ groupId: this.groupId }, this.formData)).then(() => {
    ...
    async loadFormData() {
      this.loading = true
      await this.$api.baseData.product.propertyItem.get(this.itemId).then(data => {
        this.formData = data
      }).finally(() => {
        this.loading = false
      })
