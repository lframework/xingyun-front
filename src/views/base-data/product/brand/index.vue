<template>
  <div v-permission="['base-data:product:brand:query']" class="app-container">
    ...
    >
      <template v-slot:form>
        <j-border>
          <j-form>
            <j-form-item label="品牌编号" :span="6">
              <el-input v-model="searchFormData.code" clearable />
            </j-form-item>
            <j-form-item label="品牌名称" :span="6">
      ...
      <template v-slot:toolbar_buttons>
        <el-form :inline="true">
          ...
          <el-form-item v-permission="['base-data:product:brand:modify']">
      ...
      <template v-slot:action_default="{ row }">
        <el-button v-permission="['base-data:product:brand:query']" type="text" icon="el-icon-view" @click="e => { id = row.id;$refs.viewDialog.openDialog() }">查看</el-button>
        <el-button v-permission="['base-data:product:brand:modify']" type="text" icon="el-icon-edit" @click="e => { id = row.id;$refs.updateDialog.openDialog() }">修改</el-button>
      </template>
    </vxe-grid>
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        { field: 'code', title: '品牌编号', width: 120 },
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({ page, sorts, filters }) => {
            return this.$api.baseData.product.brand.query(this.buildQueryParams(page))
          }
        }
      }
    }
  ...
  methods: {
    ...
    batchUnable() {
      ...
      this.$msg.confirm('是否确定停用选择的品牌？').then(() => {
        ...
        this.$api.baseData.product.brand.batchUnable(ids).then(data => {
    ...
    batchEnable() {
      ...
      this.$msg.confirm('是否确定启用选择的品牌？').then(() => {
        this.loading = true
        const ids = records.map(t => t.id)
        this.$api.baseData.product.brand.batchEnable(ids).then(data => {
