...
<script>
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      delProduct() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (records[i].isFixed) {
            createError('第' + (i + 1) + '行商品是零售订单中的商品，不允许删除！');
            return;
          }
        }
      ...
      createScannedRow(product, outNum = 1) {
      ...
      async consumeScanQueue() {
        ...
        try {
          const products = await api.searchRetailProducts(this.formData.scId, code);
          if (isEmpty(products)) {
      ...
      validData() {
        ...
        if (this.retailConfig.retailOutSheetRequireMember && isEmpty(this.formData.memberId)) {
          createError('会员不允许为空！');
          return false;
        }
      ...
      directApprovePassOrder() {
        ...
        createConfirm('对零售出库单执行审核通过操作？').then(() => {
          this.loading = true;
          api
