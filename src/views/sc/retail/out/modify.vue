<template>
  <div v-if="visible" class="app-container">
    <div v-permission="['retail:out:modify']" v-loading="loading">
      <j-border>
        <j-form>
          <j-form-item label="仓库" :span="6" required>
            <store-center-selector
              v-model="formData.sc"
            />
          </j-form-item>
          <j-form-item label="会员" :span="6" required>
            <member-selector
          ...
          <j-form-item label="付款日期" :span="6" required>
            <el-date-picker
      ...
      <div style="text-align: center;">
        <el-button v-permission="['retail:out:modify']" type="primary" :loading="loading" @click="updateOrder">保存</el-button>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.$api.sc.retail.outSheet.get(this.id).then(res => {
        if (!this.$enums.RETAIL_OUT_SHEET_STATUS.CREATED.equalsCode(res.status) && !this.$enums.RETAIL_OUT_SHEET_STATUS.APPROVE_REFUSE.equalsCode(res.status)) {
          this.$msg.error('零售出库单已审核通过，无法修改！')
          this.closeDialog()
          return
        }
        this.formData = {
    ...
    updateOrder() {
      ...
      this.loading = true
      this.$api.sc.retail.outSheet.updateOrder(params).then(res => {
