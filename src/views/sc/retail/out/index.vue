<template>
  <div>
    <div v-permission="['retail:out:query']">
        ...
        >
          ...
          <template #toolbar_buttons>
            <a-space>
              ...
              >
              <a-button
                v-permission="['retail:out:export']"
...
<script>
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      // 删除订单
      deleteOrder(row) {
        this.$msg.createConfirm('对选中的零售出库单执行删除操作？').then(() => {
          this.loading = true;
          api
            .deleteById(row.id)
            .then(() => {
      ...
      batchDelete() {
        ...
        if (this.$utils.isEmpty(records)) {
          this.$msg.createError('请选择要执行操作的零售出库单！');
          return;
        }

        for (let i = 0; i < records.length; i++) {
          if (this.$enums.RETAIL_OUT_SHEET_STATUS.APPROVE_PASS.equalsCode(records[i].status)) {
            this.$msg.createError('第' + (i + 1) + '个零售出库单已审核通过，不允许执行删除操作！');
            return;
          }
        }

        this.$msg.createConfirm('对选中的零售出库单执行批量删除操作？').then(() => {
          this.loading = true;
          api
      ...
      batchApprovePass() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (this.$enums.RETAIL_OUT_SHEET_STATUS.APPROVE_PASS.equalsCode(records[i].status)) {
            this.$msg.createError('第' + (i + 1) + '个零售出库单已审核通过，不允许继续执行审核！');
            return;
          }
        }

        this.$msg.createConfirm('对选中的零售出库单执行审核通过操作？').then(() => {
          this.loading = true;
          api
            .batchApprovePass({
      ...
      batchApproveRefuse() {
        ...
        for (let i = 0; i < records.length; i++) {
          ...
          if (this.$enums.RETAIL_OUT_SHEET_STATUS.APPROVE_REFUSE.equalsCode(records[i].status)) {
            this.$msg.createError('第' + (i + 1) + '个零售出库单已审核拒绝，不允许继续执行审核！');
            return;
          }
        }
