    ...
    <div v-permission="['retail:return:add']" v-loading="loading">
      ...
      >
        <a-space direction="vertical" style="width: 100%">
          <a-alert
            message="连续扫码录入"
            description="请先选择零售出库单后再扫码。扫描成功后会保留弹窗，可直接继续扫描下一件商品。"
            type="info"
            show-icon
          />
          <a-input
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      async submitScanCode() {
        if (isEmpty(this.formData.outSheetId)) {
          createError('请先选择零售出库单！');
          this.setScanMessage('请先选择零售出库单后再扫码！', 'warning');
          return;
        }
      ...
      async consumeScanQueue() {
        ...
        try {
          const products = await outApi.searchRetailProducts(this.formData.scId, code, true);
          if (isEmpty(products)) {
