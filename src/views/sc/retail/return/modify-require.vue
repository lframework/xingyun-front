    ...
    methods: {
        ...
        api
          ...
          .then((res) => {
            if (
              ...
              this.$msg.createError('零售退货单已审核通过，无法修改！');
              this.closeDialog();
              return;
            }
      ...
      addProduct() {
        if (this.$utils.isEmpty(this.formData.outSheet)) {
          this.$msg.createError('请先选择零售出库单！');
          return;
        }
      ...
      delProduct() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (records[i].isFixed) {
            this.$msg.createError('第' + (i + 1) + '行商品是零售出库单中的商品，不允许删除！');
            return;
          }
        }
      ...
      validData() {
        ...
        if (
          ...
          this.$utils.isEmpty(this.formData.member.id)
        ) {
        ...
        if (this.$utils.isEmpty(this.formData.outSheet.id)) {
          this.$msg.createError('零售出库单不允许为空！');
          return false;
        }
