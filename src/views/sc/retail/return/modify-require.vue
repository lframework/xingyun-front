  ...
  <div class="app-card-container">
    <div v-permission="['retail:return:modify']" v-loading="loading">
      ...
      <div style="text-align: center; background-color: #ffffff; padding: 8px 0">
        <a-space>
          <a-button
            v-permission="['retail:return:modify']"
            type="primary"
            :loading="loading"
            @click="updateOrder"
            >保存</a-button
...
<script>
  ...
  export default defineComponent({
    ...
    methods: {
        ...
        api
          ...
          .then((res) => {
            ...
            tableData.forEach((item) => {
              item.isFixed = !isEmpty(item.outSheetDetailId);
              if (item.isFixed) {
      ...
      // 新增商品
      addProduct() {
        if (isEmpty(this.formData.outSheet)) {
      ...
      queryProduct(queryString, row) {
        ...
        outApi.searchRetailProducts(this.formData.sc.id, queryString, true).then((res) => {
          row.products = res;
      ...
      validData() {
        ...
        if (this.retailConfig.retailReturnRequireMember && isEmpty(this.formData.member.id)) {
          createError('会员不允许为空！');
          return false;
        }
        ...
        if (isEmpty(this.formData.outSheet.id)) {
