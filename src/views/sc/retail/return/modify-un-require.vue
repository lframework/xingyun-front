    ...
    <div v-permission="['retail:return:modify']" v-loading="loading">
      <j-border>
        <j-form>
          ...
          <j-form-item v-if="$enums.RETAIL_RETURN_STATUS.APPROVE_PASS.equalsCode(formData.status) || $enums.RETAIL_RETURN_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" label="审核人">
            <span>{{ formData.approveBy }}</span>
          </j-form-item>
          <j-form-item v-if="$enums.RETAIL_RETURN_STATUS.APPROVE_PASS.equalsCode(formData.status) || $enums.RETAIL_RETURN_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" label="审核时间" :span="16">
      ...
      <div style="text-align: center;">
        <el-button v-permission="['retail:return:modify']" type="primary" :loading="loading" @click="updateOrder">保存</el-button>
...
export default {
  ...
  components: {
    StoreCenterSelector, MemberSelector, UserSelector, SupplierSelector, BatchAddProduct
  },
  props: {
    id: {
      type: String,
  ...
  methods: {
    ...
    loadData() {
      this.loading = true
      this.$api.sc.retail.retailReturn.get(this.id).then(res => {
        if (!this.$enums.RETAIL_RETURN_STATUS.CREATED.equalsCode(res.status) && !this.$enums.RETAIL_RETURN_STATUS.APPROVE_REFUSE.equalsCode(res.status)) {
          this.$msg.error('零售退货单已审核通过，无法修改！')
          this.closeDialog()
          return
        }
        this.formData = {
    ...
    handleSelectProduct(index, value) {
      ...
      this.taxPriceInput(this.tableData[index], this.tableData[index].retailPrice)
    },
    // 删除商品
    delProduct() {
    ...
    validData() {
      ...
      for (let i = 0; i < this.tableData.length; i++) {
        ...
        if (product.isGift) {
          if (parseFloat(product.retailPrice) !== 0) {
            this.$msg.error('第' + (i + 1) + '行商品价格必须等于0！')
            return false
          }
        } else {
          if (!this.$utils.isFloatGtZero(product.retailPrice)) {
    ...
    updateOrder() {
      ...
      this.loading = true
      this.$api.sc.retail.retailReturn.updateOrder(params).then(res => {
