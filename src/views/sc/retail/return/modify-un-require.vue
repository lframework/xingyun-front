    ...
    methods: {
      ...
      batchInputReturnNum() {
        ...
        createPrompt('请输入退货数量', {
          ...
          inputErrorMessage: '退货数量必须是数字并且大于0',
          title: '批量录入数量',
          required: true,
        }).then(({ value }) => {
          records.forEach((t) => {
      ...
      validData() {
        ...
        for (let i = 0; i < this.tableData.length; i++) {
          ...
          if (isEmpty(product.retailPrice)) {
            createError('第' + (i + 1) + '行商品价格不允许为空！');
            return false;
          }

          if (!isFloat(product.retailPrice)) {
          ...
          if (product.isGift) {
            ...
            if (!isFloatGtZero(product.retailPrice)) {
              createError('第' + (i + 1) + '行商品价格必须大于0！');
              return false;
            }
          }

          if (!isNumberPrecision(product.retailPrice, 6)) {
