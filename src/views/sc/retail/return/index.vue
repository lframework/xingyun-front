      ...
      >
            ...
            <j-form>
              ...
              <j-form-item label="审核状态">
                <el-select v-model="searchFormData.status" placeholder="全部" clearable>
                  <el-option v-for="item in $enums.RETAIL_RETURN_STATUS.values()" :key="item.code" :label="item.desc" :value="item.code" />
                </el-select>
              </j-form-item>
              ...
              <j-form-item label="零售出库单号">
                <el-input v-model="searchFormData.outSheetCode" clearable />
              </j-form-item>
        ...
        <template v-slot:toolbar_buttons>
          <el-form :inline="true">
            ...
            <el-form-item v-permission="['retail:return:add']">
              <el-button type="primary" icon="el-icon-plus" @click="openAddDialog">新增</el-button>
            </el-form-item>
            <el-form-item v-permission="['retail:return:approve']">
            ...
            <el-form-item v-permission="['retail:return:delete']">
              <el-button type="danger" icon="el-icon-delete" @click="batchDelete">批量删除</el-button>
            </el-form-item>
            <el-form-item v-permission="['retail:return:export']">
        ...
        <template v-slot:outSheetCode_default="{ row }">
          ...
          <span v-else>
            <el-button v-permission="['retail:out:query']" type="text" @click="viewOutSheetDetail(row.outSheetId)">{{ row.outSheetCode }}</el-button>
        ...
        <template v-slot:action_default="{ row }">
          <el-button v-permission="['retail:return:query']" type="text" icon="el-icon-view" @click="e => { id = row.id;$refs.viewDialog.openDialog() }">查看</el-button>
          <el-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['retail:return:approve']" type="text" icon="el-icon-s-check" @click="e => { id = row.id;visible=false;$nextTick(() => $refs.approveDialog.openDialog()) }">审核</el-button>
          <el-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['retail:return:modify']" type="text" icon="el-icon-edit" @click="openModifyDialog(row)">修改</el-button>
          <el-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['retail:return:delete']" type="text" icon="el-icon-delete" @click="deleteOrder(row)">删除</el-button>
        </template>
      </vxe-grid>
...
export default {
  ...
  methods: {
    ...
    viewOutSheetDetail(id) {
      this.outSheetId = id
      this.$refs.viewOutSheetDetailDialog.openDialog()
    }
  }
}
</script>
