<template>
  <div>
    <div v-show="visible" v-permission="['retail:return:query']" class="app-container">
      <!-- 数据列表 -->
      <vxe-grid
        ref="grid"
        resizable
      ...
      >
            ...
            <j-form @collapse="$refs.grid.refreshColumn()">
              ...
              <j-form-item label="状态">
                <a-select v-model="searchFormData.status" placeholder="全部" allow-clear>
                  <a-select-option v-for="item in $enums.RETAIL_RETURN_STATUS.values()" :key="item.code" :value="item.code">{{ item.desc }}</a-select-option>
                </a-select>
              </j-form-item>
        ...
        <template v-slot:toolbar_buttons>
          <a-space>
            ...
            <a-button v-permission="['retail:return:add']" type="primary" icon="plus" @click="openAddDialog">新增</a-button>
            <a-button v-permission="['retail:return:approve']" icon="check" @click="batchApprovePass">审核通过</a-button>
            <a-button v-permission="['retail:return:approve']" icon="close" @click="batchApproveRefuse">审核拒绝</a-button>
            <a-button v-permission="['retail:return:delete']" type="danger" icon="delete" @click="batchDelete">批量删除</a-button>
            <a-button v-permission="['retail:return:export']" icon="download" @click="exportList">导出</a-button>
          </a-space>
        </template>
        ...
        <template v-slot:outSheetCode_default="{ row }">
          ...
          <span v-else>
            <a v-permission="['retail:out:query']" @click="viewOutSheetDetail(row.outSheetId)">{{ row.outSheetCode }}</a>
        ...
        <template v-slot:action_default="{ row }">
          <a-space>
            <a-button v-permission="['retail:return:query']" type="link" @click="e => { id = row.id;$nextTick(() => $refs.viewDialog.openDialog()) }">查看</a-button>
            <a-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['retail:return:approve']" type="link" @click="e => { id = row.id;visible=false;$nextTick(() => $refs.approveDialog.openDialog()) }">审核</a-button>
            <a-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['retail:return:modify']" type="link" @click="openModifyDialog(row)">修改</a-button>
            <a-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['retail:return:delete']" type="link" class="ant-btn-link-danger" @click="deleteOrder(row)">删除</a-button>
          </a-space>
        </template>
      </vxe-grid>
...
export default {
  name: 'RetailReturn',
  components: {
    AddRequire, AddUnRequire, ModifyRequire, ModifyUnRequire, Detail, Approve, StoreCenterSelector, MemberSelector, UserSelector, ApproveRefuse, OutSheetDetail
  },
  data() {
    return {
      loading: false,
      ...
      tableColumn: [
        ...
        { field: 'status', title: '状态', width: 100, formatter: ({ cellValue }) => { return this.$enums.RETAIL_RETURN_STATUS.getDesc(cellValue) } },
        ...
        { field: 'outSheetCode', title: '零售出库单号', width: 180, slots: { default: 'outSheetCode_default' }},
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({ page, sorts, filters }) => {
            return this.$api.sc.retail.retailReturn.query(this.buildQueryParams(page))
          }
        }
      }
    }
  ...
  methods: {
    ...
    openAddDialog() {
      this.loading = true
      this.$api.sc.retail.retailConfig.get().then(res => {
        this.visible = false
    ...
    // 删除订单
    deleteOrder(row) {
      this.$msg.confirm('对选中的零售退货单执行删除操作？').then(() => {
        this.loading = true
        this.$api.sc.retail.retailReturn.deleteOrder({
          id: row.id
        }).then(() => {
    ...
    batchApproveRefuse() {
      ...
      for (let i = 0; i < records.length; i++) {
        if (this.$enums.PURCHASE_ORDER_STATUS.APPROVE_PASS.equalsCode(records[i].status)) {
          this.$msg.error('第' + (i + 1) + '个零售退货单已审核通过，不允许继续执行审核！')
          return
        }

        if (this.$enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(records[i].status)) {
          this.$msg.error('第' + (i + 1) + '个零售退货单已审核拒绝，不允许继续执行审核！')
          return
        }
      }
    ...
    doApproveRefuse(reason) {
      ...
      this.loading = true
      this.$api.sc.retail.retailReturn.batchApproveRefuseOrder({
        ids: records.map(item => item.id),
        refuseReason: reason
      }).then(() => {
    ...
    exportList() {
      this.loading = true
      this.$api.sc.retail.retailReturn.exportList(this.buildQueryParams({})).then(() => {
