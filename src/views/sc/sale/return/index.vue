      ...
      >
            ...
            <j-form>
              ...
              <j-form-item label="审核状态">
                <el-select v-model="searchFormData.status" placeholder="全部" clearable>
                  <el-option v-for="item in $enums.SALE_RETURN_STATUS.values()" :key="item.code" :label="item.desc" :value="item.code" />
                </el-select>
              </j-form-item>
        ...
        <template v-slot:toolbar_buttons>
          <el-form :inline="true">
            ...
            <el-form-item v-permission="['sale:return:add']">
              <el-button type="primary" icon="el-icon-plus" @click="openAddDialog">新增</el-button>
            </el-form-item>
            <el-form-item v-permission="['sale:return:approve']">
            ...
            <el-form-item v-permission="['sale:return:delete']">
              <el-button type="danger" icon="el-icon-delete" @click="batchDelete">批量删除</el-button>
            </el-form-item>
            <el-form-item v-permission="['sale:return:export']">
        ...
        <template v-slot:outSheetCode_default="{ row }">
          ...
          <span v-else>
            <el-button v-permission="['sale:out:query']" type="text" @click="viewOutSheetDetail(row.outSheetId)">{{ row.outSheetCode }}</el-button>
        ...
        <template v-slot:action_default="{ row }">
          <el-button v-permission="['sale:return:query']" type="text" icon="el-icon-view" @click="e => { id = row.id;$refs.viewDialog.openDialog() }">查看</el-button>
          <el-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['sale:return:approve']" type="text" icon="el-icon-s-check" @click="e => { id = row.id;visible=false;$nextTick(() => $refs.approveDialog.openDialog()) }">审核</el-button>
          <el-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['sale:return:modify']" type="text" icon="el-icon-edit" @click="openModifyDialog(row)">修改</el-button>
          <el-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['sale:return:delete']" type="text" icon="el-icon-delete" @click="deleteOrder(row)">删除</el-button>
        </template>
      </vxe-grid>
