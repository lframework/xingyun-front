<template>
  <div>
    <div v-permission="['sale:return:query']" class="app-container">
      <!-- 数据列表 -->
      <vxe-grid
        id="SaleReturn"
        ref="grid"
      ...
      >
        ...
        <template v-slot:action_default="{ row }">
          ...
          <a-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['sale:return:approve']" type="link" @click="$router.push('/sale/return/approve/' + row.id)">审核</a-button>
...
export default {
  name: 'SaleReturn',
  components: {
    Detail, StoreCenterSelector, CustomerSelector, UserSelector, ApproveRefuse, OutSheetDetail
  },
  data() {
    return {
      loading: false,
