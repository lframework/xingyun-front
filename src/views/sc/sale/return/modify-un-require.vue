  ...
  <div v-if="visible" class="app-container">
    <div v-permission="['sale:return:modify']" v-loading="loading">
      ...
      <div style="text-align: center; background-color: #FFFFFF;padding: 8px 0;">
        <a-space>
          <a-button v-permission="['sale:return:modify']" type="primary" :loading="loading" @click="updateOrder">保存</a-button>
...
export default {
  ...
  components: {
    StoreCenterSelector, CustomerSelector, UserSelector, SupplierSelector, BatchAddProduct
  },
  props: {
    id: {
      type: String,
  ...
  methods: {
    ...
    validData() {
      ...
      for (let i = 0; i < this.tableData.length; i++) {
        ...
        if (product.isGift) {
          if (parseFloat(product.salePrice) !== 0) {
            this.$msg.error('第' + (i + 1) + '行商品价格必须等于0！')
            return false
          }
        } else {
          if (!this.$utils.isFloatGtZero(product.salePrice)) {
