  ...
  <div v-if="visible" class="app-container">
    <div v-permission="['sale:out:add']" v-loading="loading">
      ...
      <div style="text-align: center;">
        ...
        <el-button v-permission="['sale:out:approve']" type="primary" :loading="loading" @click="redirectApprovePassOrder">审核通过</el-button>
...
export default {
  ...
  methods: {
    ...
    createOrder() {
      ...
      this.loading = true
      this.$api.sc.sale.outSheet.createOrder(params).then(res => {
    ...
    redirectApprovePassOrder() {
      ...
      this.$msg.confirm('对销售出库单执行审核通过操作？').then(() => {
        this.loading = true
        this.$api.sc.sale.outSheet.redirectApprovePassOrder(params).then(res => {
    ...
    beforeSelectSc() {
      ...
      this.$msg.error('由于“销售出库单关联销售订单”，不允许修改仓库！')
      return false
    },
    beforeSelectCustomer() {
      ...
      this.$msg.error('由于“销售出库单关联销售订单”，不允许修改客户！')
      return false
    },
    beforeSelectSaler() {
