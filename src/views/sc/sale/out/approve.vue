<template>
  <div v-if="visible" class="app-container">
    <div v-permission="['sale:out:approve']" v-loading="loading">
      ...
      <div v-if="$enums.SALE_OUT_SHEET_STATUS.CREATED.equalsCode(formData.status) || $enums.SALE_OUT_SHEET_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" style="text-align: center; background-color: #FFFFFF;padding: 8px 0;">
        <a-space>
          <a-button v-permission="['sale:out:approve']" type="primary" :loading="loading" @click="approvePassOrder">审核通过</a-button>
          <a-button v-if="$enums.SALE_OUT_SHEET_STATUS.CREATED.equalsCode(formData.status)" v-permission="['sale:out:approve']" type="danger" :loading="loading" @click="approveRefuseOrder">审核拒绝</a-button>
