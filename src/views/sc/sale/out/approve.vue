<template>
  <div v-if="visible" class="app-container">
    <div v-permission="['sale:out:approve']" v-loading="loading">
        ...
        <j-form>
          ...
          <j-form-item label="销售订单" :span="16">
            <div v-if="!$utils.isEmpty(formData.saleOrderCode)">
              <el-button v-permission="['sale:order:query']" type="text" @click="e => $refs.viewSaleOrderDetailDialog.openDialog()">{{ formData.saleOrderCode }}</el-button>
      ...
      <div v-if="$enums.SALE_OUT_SHEET_STATUS.CREATED.equalsCode(formData.status) || $enums.SALE_OUT_SHEET_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" style="text-align: center;">
        <el-button v-permission="['sale:out:approve']" type="primary" :loading="loading" @click="approvePassOrder">审核通过</el-button>
        <el-button v-if="$enums.SALE_OUT_SHEET_STATUS.CREATED.equalsCode(formData.status)" v-permission="['sale:out:approve']" type="danger" :loading="loading" @click="approveRefuseOrder">审核拒绝</el-button>
  ...
  methods: {
    ...
    loadData() {
      ...
      this.$api.sc.sale.outSheet.get(this.id).then(res => {
        if (!this.$enums.SALE_OUT_SHEET_STATUS.CREATED.equalsCode(res.status) && !this.$enums.SALE_OUT_SHEET_STATUS.APPROVE_REFUSE.equalsCode(res.status)) {
          this.$msg.error('销售出库单已审核通过，无需重复审核！')
          this.closeDialog()
          return
        }
        this.formData = {
    ...
    approvePassOrder() {
      ...
      if (!this.$utils.isEmpty(unValidStockNumArr)) {
        this.$msg.error('商品（' + unValidStockNumArr[0].productCode + '）' + unValidStockNumArr[0].productName + '当前库存为' + unValidStockNumArr[0].stockNum + '，总出库数量为' + unValidStockNumArr[0].outNum + '，无法完成销售出库！')
        return false
      }

      this.$msg.confirm('对销售出库单执行审核通过操作？').then(() => {
        this.loading = true
        this.$api.sc.sale.outSheet.approvePassOrder({
          id: this.id,
    ...
    doApproveRefuse(reason) {
      this.loading = true
      this.$api.sc.sale.outSheet.approveRefuseOrder({
        id: this.id,
        refuseReason: reason
      }).then(() => {
