  ...
  <div class="app-card-container">
    <div v-permission="['stock:take:pre:modify']" v-loading="loading">
      ...
      <div style="text-align: center; background-color: #ffffff; padding: 8px 0">
        <a-space>
          <a-button
            v-permission="['stock:take:pre:modify']"
            type="primary"
            :loading="loading"
            @click="submit"
            >保存</a-button
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      handleSelectProduct(index, value, row) {
        value =
          typeof value === 'string' && row
            ? row.products.filter((item) => item.skuId === value)[0]
            : value;
        for (let i = 0; i < this.tableData.length; i++) {
          const data = this.tableData[i];
          if (data.skuId === value.skuId) {
            if (i === index) {
      ...
      batchAddProduct(productList) {
        ...
        productList.forEach((item) => {
          if (isEmpty(this.tableData.filter((data) => item.skuId === data.skuId))) {
