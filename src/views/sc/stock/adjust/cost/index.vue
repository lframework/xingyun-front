<template>
  <div>
    <div v-permission="['stock:adjust:cost:query']">
        ...
        >
              ...
              <j-form label-width="80px" @collapse="$refs.grid.refreshColumn()">
                ...
                <j-form-item label="状态">
                  <a-select v-model:value="searchFormData.status" placeholder="全部" allow-clear>
                    <a-select-option
                      v-for="item in $enums.STOCK_COST_ADJUST_SHEET_STATUS.values()"
                      :key="item.code"
                      :value="item.code"
          ...
          <template #toolbar_buttons>
            <a-space>
              ...
              <a-button
                v-permission="['stock:adjust:cost:add']"
                type="primary"
                :icon="h(PlusOutlined)"
                @click="$router.push('/stock/stock-adjust/cost/add')"
                >新增</a-button
              >
              <a-button
                v-permission="['stock:adjust:cost:approve']"
              ...
              >
              <a-button
                v-permission="['stock:adjust:cost:delete']"
                danger
              ...
              >
              <a-button
                v-permission="['stock:adjust:cost:export']"
...
<script>
  ...
  export default defineComponent({
      ...
      return {
        ...
        tableColumn: [
          ...
          { field: 'updateBy', title: '操作人', width: 100 },
          {
            ...
            width: 100,
            formatter: ({ cellValue }) => {
              return this.$enums.STOCK_COST_ADJUST_SHEET_STATUS.getDesc(cellValue);
            },
          },
    ...
    methods: {
      ...
      deleteRow(id) {
        this.$msg.createConfirm('对选中的库存成本调整单执行删除操作？').then(() => {
          this.loading = true;
          api
            .deleteById(id)
            .then(() => {
      ...
      batchApprovePass() {
        ...
        if (this.$utils.isEmpty(records)) {
          this.$msg.createError('请选择要执行操作的库存成本调整单！');
          return;
        }

        for (let i = 0; i < records.length; i++) {
          if (
            this.$enums.STOCK_COST_ADJUST_SHEET_STATUS.APPROVE_PASS.equalsCode(records[i].status)
          ) {
            this.$msg.createError(
              '第' + (i + 1) + '个库存成本调整单已审核通过，不允许继续执行审核！',
            );
            return;
          }
        }

        this.$msg.createConfirm('对选中的库存成本调整单执行审核通过操作？').then(() => {
          this.loading = true;
          api
            .batchApprovePass({
      ...
      batchApproveRefuse() {
        ...
        for (let i = 0; i < records.length; i++) {
          ...
          if (
            this.$enums.STOCK_COST_ADJUST_SHEET_STATUS.APPROVE_REFUSE.equalsCode(records[i].status)
          ) {
            this.$msg.createError(
              '第' + (i + 1) + '个库存成本调整单已审核拒绝，不允许继续执行审核！',
            );
            return;
          }
        }
      ...
      batchDelete() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (
            ...
            this.$msg.createError(
              '第' + (i + 1) + '个库存成本调整单已审核通过，不允许执行删除操作！',
            );
            return;
          }
        }

        this.$msg.createConfirm('对选中的库存成本调整单执行批量删除操作？').then(() => {
          this.loading = true;
          api
      ...
      createActions(row) {
        return [
          ...
          {
            permission: ['stock:adjust:cost:approve'],
            label: '审核',
            ifShow: () => {
              return (
                this.$enums.STOCK_COST_ADJUST_SHEET_STATUS.CREATED.equalsCode(row.status) ||
                this.$enums.STOCK_COST_ADJUST_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)
              );
            },
            onClick: () => {
              this.$router.push('/stock/stock-adjust/cost/approve/' + row.id);
            },
          },
          {
            permission: ['stock:adjust:cost:modify'],
            label: '修改',
            ifShow: () => {
              return (
                this.$enums.STOCK_COST_ADJUST_SHEET_STATUS.CREATED.equalsCode(row.status) ||
            ...
            onClick: () => {
              this.$router.push('/stock/stock-adjust/cost/modify/' + row.id);
            },
          },
          {
            permission: ['stock:adjust:cost:delete'],
            label: '删除',
            color: 'error',
            ifShow: () => {
              return (
