<template>
  <div v-if="visible" class="app-container">
    <div v-permission="['purchase:receive:approve']" v-loading="loading">
      ...
      <div v-if="$enums.RECEIVE_SHEET_STATUS.CREATED.equalsCode(formData.status) || $enums.RECEIVE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" style="text-align: center;">
        <el-button v-permission="['purchase:receive:approve']" type="primary" :loading="loading" @click="approvePassOrder">审核通过</el-button>
        <el-button v-if="$enums.RECEIVE_SHEET_STATUS.CREATED.equalsCode(formData.status)" v-permission="['purchase:receive:approve']" type="danger" :loading="loading" @click="approveRefuseOrder">审核拒绝</el-button>
