    ...
    <div v-show="visible" v-permission="['purchase:return:query']" class="app-container">
      ...
      >
        ...
        <template v-slot:action_default="{ row }">
          ...
          <a-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['purchase:return:approve']" type="link" @click="e => { id = row.id;visible=false;$nextTick(() => $refs.approveDialog.openDialog()) }">审核</a-button>
