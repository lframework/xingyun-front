<template>
  <div>
    <div v-show="visible" v-permission="['purchase:return:query']" class="app-container">
      <!-- 数据列表 -->
      <vxe-grid
        ref="grid"
        resizable
      ...
      >
            ...
            <j-form>
              ...
              <j-form-item label="审核状态">
                <el-select v-model="searchFormData.status" placeholder="全部" clearable>
                  <el-option v-for="item in $enums.PURCHASE_RETURN_STATUS.values()" :key="item.code" :label="item.desc" :value="item.code" />
                </el-select>
              </j-form-item>
              ...
              <j-form-item label="采购收货单号">
                <el-input v-model="searchFormData.receiveSheetCode" clearable />
              </j-form-item>
        ...
        <template v-slot:toolbar_buttons>
          <el-form :inline="true">
            ...
            <el-form-item v-permission="['purchase:return:add']">
              <el-button type="primary" icon="el-icon-plus" @click="openAddDialog">新增</el-button>
            </el-form-item>
            <el-form-item v-permission="['purchase:return:approve']">
            ...
            <el-form-item v-permission="['purchase:return:delete']">
              <el-button type="danger" icon="el-icon-delete" @click="batchDelete">批量删除</el-button>
            </el-form-item>
            <el-form-item v-permission="['purchase:return:export']">
        ...
        <template v-slot:receiveSheetCode_default="{ row }">
          ...
          <span v-else>
            <el-button v-permission="['purchase:receive:query']" type="text" @click="viewReceiveSheetDetail(row.receiveSheetId)">{{ row.receiveSheetCode }}</el-button>
        ...
        <template v-slot:action_default="{ row }">
          <el-button v-permission="['purchase:return:query']" type="text" icon="el-icon-view" @click="e => { id = row.id;$refs.viewDialog.openDialog() }">查看</el-button>
          <el-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['purchase:return:approve']" type="text" icon="el-icon-s-check" @click="e => { id = row.id;visible=false;$nextTick(() => $refs.approveDialog.openDialog()) }">审核</el-button>
          <el-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['purchase:return:modify']" type="text" icon="el-icon-edit" @click="openModifyDialog(row)">修改</el-button>
          <el-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(row.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['purchase:return:delete']" type="text" icon="el-icon-delete" @click="deleteOrder(row)">删除</el-button>
        </template>
      </vxe-grid>
...
export default {
  name: 'PurchaseReturn',
  components: {
    AddRequire, AddUnRequire, ModifyRequire, ModifyUnRequire, Detail, Approve, StoreCenterSelector, SupplierSelector, UserSelector, ApproveRefuse, ReceiveSheetDetail
  },
  data() {
    return {
      loading: false,
  ...
  methods: {
    ...
    viewReceiveSheetDetail(id) {
      ...
      this.$refs.viewReceiveSheetDetailDialog.openDialog()
    }
  }
}
</script>
