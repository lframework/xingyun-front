<template>
  <div>
    <div v-permission="['purchase:return:query']">
        ...
        >
              ...
              <j-form @collapse="$refs.grid.refreshColumn()">
                ...
                <template #more>
                  <j-form-item label="采购收货单号">
                    <a-input v-model:value="searchFormData.receiveSheetCode" allow-clear />
                  </j-form-item>
          ...
          <template #toolbar_buttons>
            <a-space>
              ...
              >
              <a-button
                v-permission="['purchase:return:export']"
          ...
          <template #receiveSheetCode_default="{ row }">
            ...
            <span v-else>
              <a
                ...
                @click="viewReceiveSheetDetail(row.receiveSheetId)"
                >{{ row.receiveSheetCode }}</a
              >
  ...
  export default defineComponent({
    ...
    data() {
      return {
        ...
        tableColumn: [
          ...
          {
            field: 'receiveSheetCode',
            title: '采购收货单号',
            width: 180,
            slots: { default: 'receiveSheetCode_default' },
          },
    ...
    methods: {
      ...
      batchDelete() {
        ...
        this.$msg.createConfirm('对选中的采购退货单执行批量删除操作？').then(() => {
          this.loading = true;
          api
      ...
      batchApprovePass() {
        ...
        this.$msg.createConfirm('对选中的采购退货单执行审核通过操作？').then(() => {
          this.loading = true;
          api
            .batchApprovePass({
      ...
      createActions(row) {
        return [
          ...
          {
            permission: ['purchase:return:approve'],
            label: '审核',
            ifShow: () => {
              return (
          ...
          {
            permission: ['purchase:return:modify'],
            label: '修改',
            ifShow: () => {
              return (
                this.$enums.PURCHASE_RETURN_STATUS.CREATED.equalsCode(row.status) ||
