    ...
    <div v-permission="['purchase:return:add']" v-loading="loading">
      ...
      >
        <a-space direction="vertical" style="width: 100%">
          <a-alert
            message="连续扫码录入"
            description="请先选择采购收货单后再扫码。扫描成功后会保留弹窗，可直接继续扫描下一件商品。"
            type="info"
            show-icon
          />
          <a-input
...
<script>
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      // 新增商品
      addProduct() {
        if (isEmpty(this.formData.receiveSheetId)) {
      ...
      async submitScanCode() {
        if (isEmpty(this.formData.receiveSheetId)) {
          createError('请先选择采购收货单！');
          this.setScanMessage('请先选择采购收货单后再扫码！', 'warning');
          return;
        }
      ...
      beforeSelectSc() {
        ...
        createError('由于“采购退货单关联采购收货单”，不允许修改仓库！');
        return false;
      },
      beforeSelectSupplier() {
        ...
        createError('由于“采购退货单关联采购收货单”，不允许修改供应商！');
        return false;
      },
