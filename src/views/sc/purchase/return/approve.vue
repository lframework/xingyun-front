<template>
  <div v-if="visible" class="app-container">
    <div v-permission="['purchase:return:approve']" v-loading="loading">
        ...
        <j-form>
          ...
          <j-form-item label="采购收货单" :span="16">
            <div v-if="!$utils.isEmpty(formData.receiveSheetCode)">
              <el-button v-permission="['purchase:receive:query']" type="text" @click="e => $refs.viewReceiveSheetDetailDialog.openDialog()">{{ formData.receiveSheetCode }}</el-button>
              <span v-no-permission="['purchase:receive:query']">{{ formData.receiveSheetCode }}</span>
            </div>
          </j-form-item>
          <j-form-item label="审核状态">
            <span v-if="$enums.PURCHASE_RETURN_STATUS.APPROVE_PASS.equalsCode(formData.status)" style="color: #67C23A;">{{ $enums.PURCHASE_RETURN_STATUS.getDesc(formData.status) }}</span>
            <span v-else-if="$enums.PURCHASE_RETURN_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" style="color: #F56C6C;">{{ $enums.PURCHASE_RETURN_STATUS.getDesc(formData.status) }}</span>
          ...
          <j-form-item label="拒绝理由" :span="16" :content-nest="false">
            <el-input v-if="$enums.PURCHASE_RETURN_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" v-model="formData.refuseReason" readonly />
          </j-form-item>
      ...
      <div v-if="$enums.PURCHASE_RETURN_STATUS.CREATED.equalsCode(formData.status) || $enums.PURCHASE_RETURN_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" style="text-align: center;">
        <el-button v-permission="['purchase:return:approve']" type="primary" :loading="loading" @click="approvePassOrder">审核通过</el-button>
        <el-button v-if="$enums.PURCHASE_RETURN_STATUS.CREATED.equalsCode(formData.status)" v-permission="['purchase:return:approve']" type="danger" :loading="loading" @click="approveRefuseOrder">审核拒绝</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        { field: 'receiveNum', title: '采购数量', align: 'right', width: 100, formatter: ({ cellValue }) => { return this.$utils.isEmpty(cellValue) ? '-' : cellValue } },
  ...
  methods: {
    ...
    loadData() {
      ...
      this.$api.sc.purchase.purchaseReturn.get(this.id).then(res => {
        if (!this.$enums.PURCHASE_RETURN_STATUS.CREATED.equalsCode(res.status) && !this.$enums.PURCHASE_RETURN_STATUS.APPROVE_REFUSE.equalsCode(res.status)) {
          this.$msg.error('采购退货单已审核通过，无需重复审核！')
          this.closeDialog()
          return
        }
        this.formData = {
    ...
    approvePassOrder() {
      ...
      this.tableData.filter(item => this.$utils.isIntegerGtZero(item.returnNum)).forEach(item => {
      ...
      const unValidStockNumArr = checkStockNumArr.filter(item => item.stockNum < item.returnNum)
      if (!this.$utils.isEmpty(unValidStockNumArr)) {
        this.$msg.error('商品（' + unValidStockNumArr[0].productCode + '）' + unValidStockNumArr[0].productName + '当前库存为' + unValidStockNumArr[0].stockNum + '，总退货数量为' + unValidStockNumArr[0].returnNum + '，无法完成采购退货！')
        return false
      }

      this.$msg.confirm('对采购退货单执行审核通过操作？').then(() => {
        this.loading = true
        this.$api.sc.purchase.purchaseReturn.approvePassOrder({
          id: this.id,
    ...
    doApproveRefuse(reason) {
      this.loading = true
      this.$api.sc.purchase.purchaseReturn.approveRefuseOrder({
        id: this.id,
        refuseReason: reason
      }).then(() => {
