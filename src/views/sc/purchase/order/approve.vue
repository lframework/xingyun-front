<template>
  <div v-if="visible" class="app-container">
    <div v-permission="['purchase:order:approve']" v-loading="loading">
      ...
      <div v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(formData.status) || $enums.PURCHASE_ORDER_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" style="text-align: center; background-color: #FFFFFF;padding: 8px 0;">
        <a-space>
          <a-button v-permission="['purchase:order:approve']" type="primary" :loading="loading" @click="approvePassOrder">审核通过</a-button>
          <a-button v-if="$enums.PURCHASE_ORDER_STATUS.CREATED.equalsCode(formData.status)" v-permission="['purchase:order:approve']" type="danger" :loading="loading" @click="approveRefuseOrder">审核拒绝</a-button>
  ...
  methods: {
    ...
    approvePassOrder() {
      this.$msg.confirm('对采购单据执行审核通过操作？').then(() => {
        this.loading = true
        this.$api.sc.purchase.purchaseOrder.approvePassOrder({
          id: this.id,
    ...
    doApproveRefuse(reason) {
      this.loading = true
      this.$api.sc.purchase.purchaseOrder.approveRefuseOrder({
        id: this.id,
        refuseReason: reason
      }).then(() => {
