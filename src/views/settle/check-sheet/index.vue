<template>
  <div>
    <div v-permission="['settle:check-sheet:query']">
        ...
        >
              ...
              <j-form bordered @collapse="$refs.grid.refreshColumn()">
                ...
                <j-form-item label="状态">
                  <a-select v-model:value="searchFormData.status" placeholder="全部" allow-clear>
                    <a-select-option
                      v-for="item in SETTLE_CHECK_SHEET_STATUS.values()"
                      :key="item.code"
                      :value="item.code"
          ...
          <template #toolbar_buttons>
            <a-space>
              ...
              >
              <a-button
                v-permission="['settle:check-sheet:delete']"
                danger
              ...
              >
              <a-button
                v-permission="['settle:check-sheet:export']"
      ...
      return {
        ...
        tableColumn: [
          ...
          { field: 'createBy', title: '操作人', width: 100 },
          {
            ...
            width: 100,
            formatter: ({ cellValue }) => {
              return SETTLE_CHECK_SHEET_STATUS.getDesc(cellValue);
            },
          },
    ...
    methods: {
      ...
      batchDelete() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (SETTLE_CHECK_SHEET_STATUS.APPROVE_PASS.equalsCode(records[i].status)) {
      ...
      batchApproveRefuse() {
        ...
        for (let i = 0; i < records.length; i++) {
          ...
          if (SETTLE_CHECK_SHEET_STATUS.APPROVE_REFUSE.equalsCode(records[i].status)) {
      ...
      createActions(row) {
        return [
          ...
          {
            permission: ['settle:check-sheet:approve'],
            label: '审核',
            ifShow: () => {
              return (
                SETTLE_CHECK_SHEET_STATUS.CREATED.equalsCode(row.status) ||
                SETTLE_CHECK_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)
              );
            },
            onClick: () => {
          ...
          {
            permission: ['settle:check-sheet:modify'],
            label: '修改',
            ifShow: () => {
              return (
                SETTLE_CHECK_SHEET_STATUS.CREATED.equalsCode(row.status) ||
