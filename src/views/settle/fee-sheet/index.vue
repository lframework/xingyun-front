<template>
  <div>
    <div v-permission="['settle:fee-sheet:query']" class="app-container">
      <!-- 数据列表 -->
      <vxe-grid
        id="SettleFeeSheet"
        ref="grid"
      ...
      >
        ...
        <template v-slot:toolbar_buttons>
          <a-space>
            ...
            <a-button v-permission="['settle:fee-sheet:add']" type="primary" icon="plus" @click="$router.push('/settle/supplier/fee-sheet/add')">新增</a-button>
        ...
        <template v-slot:action_default="{ row }">
          ...
          <a-button v-if="$enums.SETTLE_FEE_SHEET_STATUS.CREATED.equalsCode(row.status) || $enums.SETTLE_FEE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['settle:fee-sheet:approve']" type="link" @click="$router.push('/settle/supplier/fee-sheet/approve/' + row.id)">审核</a-button>
          <a-button v-if="$enums.SETTLE_FEE_SHEET_STATUS.CREATED.equalsCode(row.status) || $enums.SETTLE_FEE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['settle:fee-sheet:modify']" type="link" @click="$router.push('/settle/supplier/fee-sheet/modify/' + row.id)">修改</a-button>
