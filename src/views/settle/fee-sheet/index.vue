      ...
      >
            ...
            <j-form>
              ...
              <j-form-item label="审核状态">
                <el-select v-model="searchFormData.status" placeholder="全部" clearable>
                  <el-option v-for="item in $enums.SETTLE_FEE_SHEET_STATUS.values()" :key="item.code" :label="item.desc" :value="item.code" />
                </el-select>
              </j-form-item>
        ...
        <template v-slot:toolbar_buttons>
          <el-form :inline="true">
            ...
            <el-form-item v-permission="['settle:fee-sheet:delete']">
              <el-button type="danger" icon="el-icon-delete" @click="batchDelete">批量删除</el-button>
            </el-form-item>
            <el-form-item v-permission="['settle:fee-sheet:export']">
        ...
        <template v-slot:action_default="{ row }">
          <el-button v-permission="['settle:fee-sheet:query']" type="text" icon="el-icon-view" @click="e => { id = row.id;$refs.viewDialog.openDialog() }">查看</el-button>
          <el-button v-if="$enums.SETTLE_FEE_SHEET_STATUS.CREATED.equalsCode(row.status) || $enums.SETTLE_FEE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['settle:fee-sheet:approve']" type="text" icon="el-icon-s-check" @click="e => { id = row.id;visible=false;$nextTick(() => $refs.approveDialog.openDialog()) }">审核</el-button>
          <el-button v-if="$enums.SETTLE_FEE_SHEET_STATUS.CREATED.equalsCode(row.status) || $enums.SETTLE_FEE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['settle:fee-sheet:modify']" type="text" icon="el-icon-edit" @click="e => { id = row.id;visible = false;$nextTick(() => $refs.modifyDialog.openDialog()) }">修改</el-button>
          <el-button v-if="$enums.SETTLE_FEE_SHEET_STATUS.CREATED.equalsCode(row.status) || $enums.SETTLE_FEE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['settle:fee-sheet:delete']" type="text" icon="el-icon-delete" @click="deleteOrder(row)">删除</el-button>
        </template>
      </vxe-grid>
