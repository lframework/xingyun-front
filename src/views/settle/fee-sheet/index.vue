              ...
              <j-form bordered @collapse="$refs.grid.refreshColumn()">
                ...
                <j-form-item label="状态">
                  <a-select v-model:value="searchFormData.status" placeholder="全部" allow-clear>
                    <a-select-option
                      v-for="item in SETTLE_FEE_SHEET_STATUS.values()"
                      :key="item.code"
                      :value="item.code"
      ...
      return {
        ...
        tableColumn: [
          ...
          { field: 'createBy', title: '操作人', width: 100 },
          {
            ...
            width: 100,
            formatter: ({ cellValue }) => {
              return SETTLE_FEE_SHEET_STATUS.getDesc(cellValue);
            },
          },
    ...
    methods: {
      ...
      batchDelete() {
        ...
        for (let i = 0; i < records.length; i++) {
          if (SETTLE_FEE_SHEET_STATUS.APPROVE_PASS.equalsCode(records[i].status)) {
      ...
      batchApproveRefuse() {
        ...
        for (let i = 0; i < records.length; i++) {
          ...
          if (SETTLE_FEE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(records[i].status)) {
      ...
      createActions(row) {
        return [
          ...
          },
          {
            ...
            ifShow: () => {
              return (
                SETTLE_FEE_SHEET_STATUS.CREATED.equalsCode(row.status) ||
                SETTLE_FEE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)
              );
            },
            onClick: () => {
