<template>
  <a-modal v-model="visible" :mask-closable="false" width="40%" title="修改" :dialog-style="{ top: '20px' }" :footer="null">
    <div v-if="visible" v-permission="['settle:in-item:modify']" v-loading="loading">
  ...
  methods: {
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.loading = true
          this.$api.settle.inItem.modify(this.formData).then(() => {
    ...
    async loadFormData() {
      this.loading = true
      await this.$api.settle.inItem.get(this.id).then(data => {
        this.formData = data
      }).finally(() => {
        this.loading = false
      })
