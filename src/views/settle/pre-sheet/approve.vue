<template>
  <div v-if="visible" class="app-container">
    <div v-permission="['settle:pre-sheet:approve']" v-loading="loading">
      <j-border>
        <j-form>
          ...
          <j-form-item label="状态">
            <span v-if="$enums.SETTLE_PRE_SHEET_STATUS.APPROVE_PASS.equalsCode(formData.status)" style="color: #52C41A;">{{ $enums.SETTLE_PRE_SHEET_STATUS.getDesc(formData.status) }}</span>
            <span v-else-if="$enums.SETTLE_PRE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" style="color: #F5222D;">{{ $enums.SETTLE_PRE_SHEET_STATUS.getDesc(formData.status) }}</span>
          ...
          <j-form-item label="拒绝理由" :content-nest="false" :span="16">
            <a-input v-if="$enums.SETTLE_PRE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" v-model="formData.refuseReason" read-only />
          </j-form-item>
      ...
      <div v-if="$enums.SETTLE_PRE_SHEET_STATUS.CREATED.equalsCode(formData.status) || $enums.SETTLE_PRE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(formData.status)" style="text-align: center; background-color: #FFFFFF;padding: 8px 0;">
        <a-space>
          <a-button v-permission="['settle:pre-sheet:approve']" type="primary" :loading="loading" @click="approvePassOrder">审核通过</a-button>
          <a-button v-if="$enums.SETTLE_PRE_SHEET_STATUS.CREATED.equalsCode(formData.status)" v-permission="['settle:pre-sheet:approve']" type="danger" :loading="loading" @click="approveRefuseOrder">审核拒绝</a-button>
  ...
  methods: {
    ...
    approvePassOrder() {
      this.$msg.confirm('确定执行审核通过操作？').then(() => {
        this.loading = true
        this.$api.settle.preSheet.approvePassOrder({
          id: this.id,
    ...
    doApproveRefuse(reason) {
      this.loading = true
      this.$api.settle.preSheet.approveRefuseOrder({
        id: this.id,
        refuseReason: reason
      }).then(() => {
