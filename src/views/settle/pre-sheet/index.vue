    ...
    <div v-show="visible" v-permission="['settle:pre-sheet:query']" class="app-container">
      ...
      >
        ...
        <template v-slot:action_default="{ row }">
          ...
          <a-button v-if="$enums.SETTLE_PRE_SHEET_STATUS.CREATED.equalsCode(row.status) || $enums.SETTLE_PRE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['settle:pre-sheet:approve']" type="link" @click="e => { id = row.id;visible=false;$nextTick(() => $refs.approveDialog.openDialog()) }">审核</a-button>
          <a-button v-if="$enums.SETTLE_PRE_SHEET_STATUS.CREATED.equalsCode(row.status) || $enums.SETTLE_PRE_SHEET_STATUS.APPROVE_REFUSE.equalsCode(row.status)" v-permission="['settle:pre-sheet:modify']" type="link" @click="e => { id = row.id;visible = false;$nextTick(() => $refs.modifyDialog.openDialog()) }">修改</a-button>
